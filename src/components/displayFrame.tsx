    ...
    return (
        ...
        >
            ...
            <div>
                ...
                {response && response.type === 'image' && Array.isArray(response.content) && Array.isArray(response.prompt) && (
                    <div className="relative group mb-6 flex flex-col items-center">
                        {response.content.map((content, idx) => (
                            <div key={`image-block-${idx}`} className="w-full flex flex-col items-center mb-8">
                                {/* Display the prompt above the image */}
                                <motion.div
                                    className="mb-2 p-4 bg-gray-50 rounded-lg border border-gray-200 prose prose-sm lg:prose-base dark:prose-invert w-[90%]"
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
